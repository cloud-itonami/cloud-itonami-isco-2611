(ns legalpractice.actor
  "LegalPracticeActor — the ISCO-08 2611 community legalpractice actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one legalpractice operation request
  ...
  The unconditional invariant: the LegalPracticeAdvisor can never
  directly commit a record the LegalPracticeGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [legalpractice.advisor :as advisor]
            [legalpractice.governor :as governor]
            [legalpractice.store :as store]))

(defn build-graph
  "Build a compiled LegalPracticeActor graph. `store` implements
  `legalpractice.store/Store`. `advisor` implements
  `legalpractice.advisor/Advisor` (defaults to `mock-advisor`).
